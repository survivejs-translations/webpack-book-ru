...
Проблема может быть решена путём обработки кода через [Babel](https://babeljs.io/), известным JavaScript-компилятором, который поддерживает возможности ES2015+ и даже больше. Он похож на ESLint, поскольку он построен поверх пресетов и плагинов. Пресеты — это сборники плагинов с возможностью определять собственных.
...
## Использование Babel с настройкой Webpack
...
Подключение Babel к проекту позволяет обрабатывать конфигурацию webpack через него. Для работы это варианта имя конфигурационного файла webpack должно следовать соглашение *webpack.config.babel.js*. Пакет [interpret](https://www.npmjs.com/package/interpret) позволяет это, и он также поддерживает другие компиляторы.
...
Несмотря на то, что у вас установлен и настроен Babel, вам все равно не хватает одной важной детали: конфигурация Babel. Конфигурация может быть настроена с использованием файла *.babelrc*, поскольку другие инструменты могут использовать их.

W> Если вы попытаетесь импортировать файлы **вне** корневой директории конфигурации для обработки их через *babel-loader*, то это не сработает. Это [известная проблема](https://github.com/babel/babel-loader/issues/313), и существуют обходные пути, включая перенос файла *.babelrc* на более высоком уровень иерархии в проекте и разрешение пресетов Babel через `require.resolve` в конфигурации webpack.
...
Скорректируйте определение цели по своему усмотрению. Если вы следуете [browserslist](https://www.npmjs.com/package/browserslist), это должно работать. Вот пример конфигурации:
...
Попробуйте включить только определение, как `IE 8`, и код должен измениться соответствующим образом:
...
W> Некоторые возможности webpack, такие как *Раздление кода*, записывают код на основе `Promise` в инициализацию webpack после того, как он обработал загрузчики. Данная проблема может быть решена путём шима до того, как будет выполнен код приложения. Пример: `entry: { app: ["core-js/es6/promise", PATHS.app] }`.

## Советы по Babel

Существуют другие возможные [опций *.babelrc*](https://babeljs.io/docs/usage/options/), кроме уже рассмотренных. Подобно ESLint, *.babelrc* поддерживает [JSON5](https://www.npmjs.com/package/json5) в качестве формата конфигурации, что означает, что вы можете использовать комментарии, использовать одиночные кавычки и т.д.

Иногда вам понадобится экспериментальные возможности, соответствующие вашему проекту. Хотя вы можете найти много их в так называемых пресетах-этапах, это хорошая идея, чтобы можно было включать их один за другим и даже организовывать в собственный пресет, если вы не работаете над одноразовым проектом. Если вы ожидаете, что проект будет долгосрочным, лучше документировать функции, которые вы активно используете.
...
* [babel-plugin-import-asserts](https://www.npmjs.com/package/babel-plugin-import-asserts) утверждает, что ваш импорт определён.
* [babel-plugin-jsdoc-to-assert](https://www.npmjs.com/package/babel-plugin-jsdoc-to-assert) конвертирует [JSDoc](http://usejsdoc.org/) аннотации к исполняемым утверждениям.
...
* [babel-plugin-annotate-console-log](https://www.npmjs.com/package/babel-plugin-annotate-console-log) аннотирует вызовы `console.log` с информацией о контексте вызова, поскольку легче видеть, где они регистрировались.
* [babel-plugin-sitrep](https://www.npmjs.com/package/babel-plugin-sitrep) регистрирует все присваивания функции и печатает их.
* [babel-plugin-webpack-loaders](https://www.npmjs.com/package/babel-plugin-webpack-loaders) позволяет использовать определённые загрузки webpack через Babel.
* [babel-plugin-syntax-trailing-function-commas](https://www.npmjs.com/package/babel-plugin-syntax-trailing-function-commas) добавляет поддержку завершающей запятой для функций.
* [babel-plugin-transform-react-remove-prop-types](https://www.npmjs.com/package/babel-plugin-transform-react-remove-prop-types) позволяет удалить код `propType`, связанный с вашей продакшен-сборкой. Этот пакет также позволяет авторам компонентов генерировать код, который завернут, чтобы настройка окружения в `DefinePlugin` могла сработать, как описано в книге.
...
Babel позволяет вам контролировать, какие пресеты и плагины используются для каждого окружения с помощью [опции env](https://babeljs.io/docs/usage/babelrc/#env-option). Таким образом, вы можете управлять поведением Babel для каждой цели сборки.
...
Любые общие (shared) пресеты и плагины доступны для всех целей. Поле `env` позволяет вам дополнительно сконфигурировать вашу конфигурацию Babel.
...
T> Способ `env` работает тонко. Рассмотрите логирование `env` и убедитесь, что оно соответствует настройке Babel, или в противном случае функциональность, которую вы ожидаете, не применится к вашей сборке.
...
Babel стал незаменимым инструментом для разработчиков, учитывая, что он объединяет новые стандарты написания кода со старыми браузерами. Даже если вы ориентируетесь на современные браузеры, обработка кода через Babel — правильное решение.
